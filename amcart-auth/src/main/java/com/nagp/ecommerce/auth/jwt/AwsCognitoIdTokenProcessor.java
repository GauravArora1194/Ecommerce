...
import com.nagp.ecommerce.auth.configuration.JwtConfiguration;
...
public class AwsCognitoIdTokenProcessor {
	...
	private void verifyIfIdToken(JWTClaimsSet claims) throws Exception {
		if (!claims.getStringClaim("token_use").equals("id")) {
	...
	private String getBearerToken(String token) {
		return token.startsWith("Bearer ") ? token.substring("Bearer ".length()).trim() : token;
	}
}
